...
export default function AccountScreen() {
  ...
  const [activeTab, setActiveTab] = useState('history'); // 'history' hoặc 'contributions'
  ...
  // 1. Hàm tải dữ liệu (Lịch sử hoặc Đóng góp)
  const fetchData = async () => {
    ...
    try {
      ...
      if (activeTab === 'history') {
        ...
        // Lấy danh sách WC đã đóng góp (dựa vào field createdBy mình đã thêm ở bước trước)
      ...
      // Sắp xếp mới nhất lên đầu (Client side sort)
      if (activeTab === 'history') {
  ...
  // 3. Hàm Đổi mật khẩu (Cần xác thực lại mật khẩu cũ)
  const handleChangePassword = async () => {
    if (!user || !user.email) return;
    try {
      // Re-authenticate (Bắt buộc)
      const credential = EmailAuthProvider.credential(user.email, currentPass);
      await reauthenticateWithCredential(user, credential);
      
      // Đổi pass
      await updatePassword(user, newPass);
      Alert.alert("Thành công", "Mật khẩu đã đổi. Vui lòng đăng nhập lại!");
      auth.signOut().then(() => router.replace('/login'));
    } catch (error: any) {
      Alert.alert("Lỗi", "Mật khẩu cũ không đúng hoặc có lỗi: " + error.message);
    }
  };
  ...
  // Render từng dòng lịch sử/đóng góp
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.tabContainer}>
        ...
        >
            <Text style={[styles.tabText, activeTab === 'history' && styles.activeTabText]}>Lịch sử Check-in</Text>
        </TouchableOpacity>
        <TouchableOpacity 
            style={[styles.tabButton, activeTab === 'contributions' && styles.activeTab]} 
            onPress={() => setActiveTab('contributions')}
        >
            <Text style={[styles.tabText, activeTab === 'contributions' && styles.activeTabText]}>Đóng góp của tôi</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.listContainer}>
          {loading ? (
              ...
              <FlatList
                ...
                ListEmptyComponent={<Text style={styles.emptyText}>Trống trơn... 🍃</Text>}
            ...
            <View style={styles.modalContent}>
                ...
                <View style={styles.modalButtons}>
                    ...
                    <TouchableOpacity onPress={handleChangePassword} style={styles.confirmBtn}>
                        <Text style={{color: 'white'}}>Đổi ngay</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </View>
