...
const AMENITY_LABELS: Record<string, string> = {
  'hot_water': 'Nước nóng', 'towel': 'Khăn tắm', 'soap': 'Dầu gội/Sữa tắm',
  'hair_dryer': 'Máy sấy', 'locker': 'Tủ đồ', 'parking': 'Gửi xe',
  'wifi': 'Wifi Free', 'wc': 'Nhà vệ sinh', 'sauna': 'Xông hơi',
  'massage': 'Massage', 'laundry': 'Giặt ủi', 'shop': 'Tạp hóa',
  'charge': 'Sạc ĐT', 'accessible': 'Lối xe lăn'
};
...
export default function ToiletDetailModal({ visible, onClose, toilet }: ToiletDetailModalProps) {
  ...
  useEffect(() => {
    setMyRating(0); setReviewText(''); setIsWritingReview(false);
  }, [toilet]);

  useEffect(() => {
    if (!toilet) return;
    const q = query(collection(db, "reviews"), where("toiletId", "==", toilet.id), orderBy("createdAt", "desc"));
  ...
  const handleDirectCheckIn = () => {
    ...
    // Mở Scanner, truyền toilet data để tạo booking mới
    setScannerVisible(true);
  };

  const submitReview = async () => {
    if (myRating === 0) { Alert.alert("Khoan!", "Vui lòng chọn số sao ⭐"); return; }
    const user = auth.currentUser;
    if (!user) return;

    try {
      await addDoc(collection(db, "reviews"), {
        toiletId: toilet.id, userEmail: user.email, userName: user.displayName || 'Ẩn danh',
        userPhoto: user.photoURL, rating: myRating, comment: reviewText, createdAt: new Date().toISOString()
      });
      await updateDoc(doc(db, "toilets", toilet.id), {
        rating: ((toilet.rating || 5) * (toilet.ratingCount || 1) + myRating) / ((toilet.ratingCount || 1) + 1),
    ...
    <Modal animationType="slide" visible={visible} onRequestClose={onClose}>
      <SafeAreaView style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
            <Image source={{ uri: toilet.image || 'https://images.unsplash.com/photo-1552321554-5fefe8c9ef14?w=600' }} style={styles.heroImage} />

            <View style={styles.section}>
                <Text style={styles.title}>{toilet.name}</Text>
                <View style={styles.ratingRow}>
                   <Text style={styles.ratingBig}>{toilet.rating ? toilet.rating.toFixed(1) : 5.0}</Text>
                   <Ionicons name="star" size={16} color="#FBC02D" />
                   <Text style={styles.ratingCount}> ({toilet.ratingCount || 0} đánh giá)</Text>
                </View>
                <Text style={styles.subtitle}>📍 {formatDistance(toilet.distance)} • {toilet.address}</Text>
            </View>

            {/* ACTION BUTTONS */}
            <View style={styles.actionRow}>
                {/* Nút Đặt trước */}
                <TouchableOpacity style={[styles.mainBtn, styles.bookBtn]} onPress={handleBookingRequest}>
                ...
                {/* Nút Check-in ngay */}
                <TouchableOpacity style={[styles.mainBtn, styles.checkInBtn]} onPress={handleDirectCheckIn}>
            ...
            <View style={styles.section}>
                ...
                {!isWritingReview ? (
                   <TouchableOpacity style={styles.writeReviewBtn} onPress={() => setIsWritingReview(true)}>
                      ...
                      <Text style={{color:'#0288D1', marginLeft: 5}}>Viết đánh giá</Text>
                   </TouchableOpacity>
                ) : (
                  <View style={styles.reviewForm}>
                    <Text style={{marginBottom: 10}}>Đánh giá trải nghiệm của bạn:</Text>
                    ...
                    <TextInput style={styles.reviewInput} placeholder="Nhập bình luận..." multiline value={reviewText} onChangeText={setReviewText} />
                    <View style={{flexDirection: 'row', justifyContent: 'flex-end', gap: 10}}>
                      <TouchableOpacity onPress={() => setIsWritingReview(false)}><Text style={{color: '#666', padding: 10}}>Hủy</Text></TouchableOpacity>
                      <TouchableOpacity onPress={submitReview} style={{backgroundColor:'#0288D1', padding: 10, borderRadius: 5}}><Text style={{color:'white'}}>Gửi</Text></TouchableOpacity>
                    </View>
                  </View>
                )}
                {reviews.map((rev) => (
                  <View key={rev.id} style={styles.reviewItem}>
                    <View style={{flex: 1}}>
                       ...
                       <View style={{flexDirection:'row'}}>{[...Array(rev.rating)].map((_, i) => <Ionicons key={i} name="star" size={12} color="#FBC02D" />)}</View>
        ...
        {/* CÁC MODAL CON */}
        <BookingForm visible={bookingFormVisible} onClose={() => setBookingFormVisible(false)} toilet={toilet} />
        <QRScanner visible={scannerVisible} onClose={() => setScannerVisible(false)} toiletData={toilet} />
...
const styles = StyleSheet.create({
  ...
  mainBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', padding: 12, borderRadius: 12 },
  ...
  chip: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 15, backgroundColor: '#E3F2FD' },
  chipText: { fontSize: 12, color: '#1976D2' },
  
  sectionHeader: { fontSize: 18, fontWeight: 'bold', marginBottom: 15 },
  writeReviewBtn: { flexDirection: 'row', alignItems: 'center', marginBottom: 10 },
  reviewForm: { padding: 15, borderWidth: 1, borderColor: '#eee', borderRadius: 8, marginBottom: 15 },
  reviewInput: { borderWidth: 1, borderColor: '#ddd', borderRadius: 5, padding: 10, height: 60, textAlignVertical: 'top', marginBottom: 10 },
  reviewItem: { marginTop: 15, borderBottomWidth: 1, borderBottomColor: '#eee', paddingBottom: 15 },
